...
- **ClientHooks**：从 18 个方法扩展至 **108 个方法**，完整委托 `ForgeHooksClient`（GUI 层、盔甲/实体渲染、输入事件、工具提示、模型烘焙、雾/视口、声音、截图等）。
- **EventHooks**：新增 ~13 个缺失方法（`onFarmlandTrample`、`fireFluidPlaceBlockEvent`、`onAlterGround`、`firePlayerRespawnEvent`、`canEntityGrief`、`onEntitySize` 等）。
- **CommonHooks**：新增 ~14 个缺失方法（`onLivingConvert`、`onEntityDestroyBlock`、`onPlayerAttack`、`canLivingConvert`、`getEntitySizeForPos` 等）。
- **客户端事件包装器（~20 个）**：`RenderPlayerEvent`、`RenderNameTagEvent`、`ClientChatReceivedEvent`、`ClientPauseChangeEvent`、`TextureAtlasStitchedEvent`、`ModelEvent`、`PlaySoundEvent`、`RenderHighlightEvent`、`ScreenEvent`、`ViewportEvent` 等。
- **通用事件包装器（~40 个）**：服务器生命周期事件、`EntityLeaveLevel/Mount/MobGriefing`、`LivingDrops/EquipmentChange/AnimalTame`、`LivingEntityUseItem`（4 个子类）、`AttackEntity/AnvilRepair/ItemFished/CriticalHit`、`AdvancementEvent`（2 个子类）、`PlayerXpEvent`（3 个子类）、`ChunkEvent/ChunkWatch/ChunkData`、`CropGrowEvent`、`GrindstoneEvent` 等。
- **FMLConstructModEvent**：新增生命周期事件包装器，用于模组构造阶段。
- **注册系统**：实现完整 DataMap 系统（`DataMapStorage`、`IRegistryExtension.getDataMap()`、`IWithData.getData()`）；`DeferredHolder.is(TagKey)` / `tags()` 通过 `BuiltInRegistries` 解析；全部 4 种 HolderSetType 编解码器已完整实现。
- **网络 / Payload**：`ClientPayloadContext.reply()` 和 `ServerPayloadContext.reply()` 已正确委托。
- **扩展 / 通用 API**：`CommonHooks.getTagFromVanillaTier()` 映射全部 6 种原版 Tier；`FarmlandWaterManager` 委托至 Forge；`IBlockEntityExtension.getPersistentData()` 逐实例缓存；`CompositeHolderSet` 支持 `homogenize()`。
- **客户端渲染**：`CompositeRenderable.render()` 和 `BakedModelRenderable.render()` 完整实现。
- **权限系统**：`PermissionAPI.getRegisteredNodes()` 由真实注册表支持，通过 `PermissionGatherEvent` 填充。

### 说明

- 上述数值属于工程估算，不等同于正式测试通过率。
- 共 766 个 Java 源文件（686 个 shim + 80 个核心），40 个 Mixin 补丁，60 个事件 wrapper 构造函数。
- 仅剩 4 个 `UnsupportedOperationException` — 全部为设计性保留（如 `PartEntity.getAddEntityPacket()`、`ClientCommandSourceStack.getServer()`）。
- 当前最大缺口：更深层的客户端渲染/模型烘焙钩子、复杂实体同步、核心 Mixin（MinecraftMixin、GameRendererMixin）、以及 NeoForge 对原版类的深层补丁行为。
